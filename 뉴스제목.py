# requests 모듈을 이용해서
# 네이버 뉴스 메인화면에서 있는뉴스들 제목 전체 가져오기

import requests

# headers 없으면 네이버가 요청 차단
headers = \
...
for i in range (count):                             # 해당 개수만큼 반복
    ...
    extract_data = source_data[: pos2]          # 앞부분부터 뒷부분까지내용 추출해서 저장

    source_data = source_data[pos2 + 1:]    # 다음 뉴스를 찾기 위해 뒷부분을 이동시키기
